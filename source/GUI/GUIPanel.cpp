...
bool GUIPanel::loadXMLFile(const char *stackPath)
{
  const char *xmlFile = MediaPathManager::lookUpMediaPath(stackPath);
  XMLStack    cfgStack;
  bool        result = false;
  if (!xmlFile)
    return Logger::writeErrorLog(String("Failed to locate the XML GUI file -> ") + xmlFile);
    
  if (cfgStack.loadXMLFile(xmlFile) != XML_SUCCESS)
    return  Logger::writeErrorLog(String("Invalid XML GUI description file -> ") + xmlFile);
...
bool GUIPanel::loadXMLSettings(XMLElement *element)
{
  ...
  if (child = element->getChildByName("description"))
    return  loadXMLFile(child->getValuec());
