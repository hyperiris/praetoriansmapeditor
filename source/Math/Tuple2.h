...
class Tuple2
{
  public:
    ...
    inline operator const T2*() const
    {
      return &x;
    }
    inline operator T2*()
    ...
    inline const T2  operator[](int i) const
    {
      return ((T2*)&x)[i];
    }
    ...
    friend ostream & operator << (ostream & out, const Tuple2<T2> &right)
    {
      return out << "Tuple(" << right.x << ", " << right.y << ")";
    }
    
    friend istream & operator >> (istream & in, Tuple2<T2> &right)
    {
