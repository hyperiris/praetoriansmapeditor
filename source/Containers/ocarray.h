...
class Array
{
  ...
  public:
    ...
    // copy constructor of all elements will be called.
    ...
    T removeLast()
    {
      return removeAt(length_==0 ? 0 : length_-1);
    }
...
class ArrayPtr
{
  ...
  public:
    ...
    // collections.
    ArrayPtr<T>& operator= (const ArrayPtr<T>& c)
    {
      a_=c.a_;
      return *this;
    }
    ...
    void removeRange(size_t i, size_t run_length)
    {
      return a_.removeRange(i, run_length);
    }
...
inline bool operator== (const Array<T>& a1, const Array<T>& a2)
{
  ...
  for (size_t ii=0; ii<a1.length(); ii++)
