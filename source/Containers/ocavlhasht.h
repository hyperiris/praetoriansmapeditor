...
class AVLHashT
{
  ...
  public:
    ...
    bool operator== (const AVLHashT<K,V,CHUNKSIZE>& t1) const
    {
      ...
      {
        Found_e found_where;
        N* node = lookup_(it.key(), found_where);
    ...
    {
      Found_e found_where;
      N* node = lookup_(target, found_where);
    ...
    void insertKeyAndValue(const K& key, const V& value)
    {
      ...
      Found_e found_where;
      N* node = lookup_(key, hashkey, found_where);
      if (found_where==FOUND)
      {
      ...
      if (found_where!=NOT_FOUND_BUCKETS)
      {
        ...
        while (current->parent()!=root_)
        {
          ...
          if (current->left()==kid)    // ... came left
          {
            ...
            {
              case -2:
                rotate_(current, kid); // Need to rotate, then done
              case  0:
                return ret_val;;    // All done
              case -1:
                continue;  // Balance okay @ -1: update parent's balance
            }
          }
          else   // ... came right
          {
    ...
    void deleteNode_(N* current)
    {
      ...
      while (current->parent()!=root_)
      {
        ...
        if (current->left()==kid)    // ... came left
        {
          ...
          {
            case +2:
              rotate_(current,current->right());// Need to rotate
              current = current->parent();// After rotate, update current
              if (current->balance==0) continue;
            case +1:
              goto breakout;    // All done
            case  0:
              continue;  // Balance okay @ 0: update parent's balance
          }
        }
        else   // ... came right
        {
          ...
          {
            case -2:
              rotate_(current,current->left()); // Need to rotate
              current = current->parent();// After rotate, update current
              if (current->balance==0) continue;
            case -1:
              goto breakout;    // All done
...
class AVLHashTIterator
{
  ...
  public:
    ...
    void reset()
    {
      node_ = current_ = (AVLNode_<K,V>*)(tree_->root_);
    }
    
    // Resets the iterator to iterate over collection c
    void reset(const AVLHashT<K,V,CHUNKSIZE>& c)
    {
      tree_=&c;
      reset();
    }
    ...
    {
      typedef AVLHashT<K, V, CHUNKSIZE> NCHT;
      return (NCHT*)tree_;
    }
...
template <class K, class V> struct AVLNR
{
  ...
  void swap(AVLNR<K,V>& rhs)
  {
    ::swap(node, rhs.node);
  }
  bool operator== (AVLNR<K,V>a2) const
  {
    return node->key==a2.node->key;
  }
  bool operator> (AVLNR<K,V>a2) const
  {
    return node->key > a2.node->key;
...
template<class K, class V>
void swap(AVLNR<K,V>& lhs, AVLNR<K,V>& rhs)
{
  lhs.swap(rhs);
}
...
class AVLHashTSortedIterator
{
  public:
    ...
    const K& key()   const
    {
      return keys_[current_].node->key;
    }
    V& value() const
    {
      return keys_[current_].node->value;
    }
