...
inline void OCInsertionSort(CONTAINER& c, int start, int length)
{
  ...
  {
    case 2:
      OCHardCodedSort2(c[start],c[start+1]);
      break;
    case 3:
      OCHardCodedSort3(c[start],c[start+1],c[start+2]);
      break;
    case 4:
      OCHardCodedSort4(c[start],c[start+1],c[start+2],c[start+3]);
      break;
    default:
      OCInsertionSortHelper_(c,c[start],start,length);
      break;
  }
}
...
tail:  // Take advantage of tail recursion to avoid a pop of stack
    ...
    frames.append(start);
    frames.append(left_wall);
    // OCQuickSort(c, start, left_wall);
    start = right_wall;
    goto tail; // OCQuickSort(c, right_wall, end);
  }
}
