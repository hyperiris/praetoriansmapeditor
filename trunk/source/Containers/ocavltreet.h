...
struct AVLTreeTNode_
{
  ...
  {
    AVLP t=(AVLP)left_;
    return t&1 ? 0 : (AVLTreeTNode_<K,V>*)t;
  }
  ...
  // Return the parent
  AVLTreeTNode_<K,V>* parent() const
  {
    return parent_;
  }
...
class AVLTreeTIterator
{
  ...
  public:
    ...
    bool next()
    {
      return (current_=current_->inorderSucc())!=0;
    }
    ...
    void reset()
    {
      current_ = (AVLTreeTNode_<K,V>*)(tree_->root_);
    }
    
    // Resets the iterator to iterate over collection c
    void reset(const AVLTreeT<K,V,OC_AVLCHUNKSIZE>& c)
    {
      tree_=&c;
      reset();
    }
    ...
    {
      typedef AVLTreeT<K, V, OC_AVLCHUNKSIZE> NCHT;
      return (NCHT*)tree_;
    }
