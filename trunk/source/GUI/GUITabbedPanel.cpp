...
bool GUITabbedPanel::loadXMLSettings(XMLElement *element)
{
  ...
  XMLElement *child = NULL, *subChild = NULL;
  ...
  if (child = element->getChildByName("drawBackground"))
    mainPanel->enableBGColor(child->getValue() == "true");
    
  if (child = element->getChildByName("drawBounds"))
    mainPanel->setVisibleBounds(child->getValue() == "true");
    
  if (child = element->getChildByName("clipSize"))
    mainPanel->setClipSize(child->getValuef());
...
void GUITabbedPanel::actionPerformed(GUIEvent &evt)
{
  ...
  {
    const Widgets &widgets = lowerPanel->getWidgets();
    const Widgets &buttons = upperPanel->getWidgets();
...
void GUITabbedPanel::render(float clockTick)
{
  ...
  const Tuple4i &mainBounds   = mainPanel->getWindowBounds();
  const Tuple3f &borderClr    = mainPanel->getBordersColor();
  
  lockItems = true;
  mainPanel->render(clockTick);
  
  if (mainPanel->boundsVisible())
  {
...
GUIPanel* GUITabbedPanel::getUpperPanel()
{
  return upperPanel;
}
